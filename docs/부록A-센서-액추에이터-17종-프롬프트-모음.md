# 센서·액추에이터 17종 전체 프롬프트 예시 모음

> 📂 **부록A / 라즈베리파이 담당 / 02의 4·5장에 없는 부품을 만들 때** · 전체 목록 [docs/README.md](README.md)

> `02-라즈베리파이5-바이브코딩-실습교재.md`의 **4장(센서 모듈 구현, Lab 2)**과 **5장(액추에이터 모듈 구현,
...
3. "🤖 생성된 소스 코드"는 정답이 아니라 **비교 기준**이다 — Antigravity가 만들어준
   코드가 이것과 크게 다르면(특히 반환하는 딕셔너리 키 이름), `02-라즈베리파이5-바이브코딩-실습교재.md`
   9.1절의 3대 검증 원칙에 따라 다시 확인한다.
4. DHT11·조도센서·서보·LED는 `02-라즈베리파이5-바이브코딩-실습교재.md` 4~5장에 이미 있으므로 여기서는
...
### A-5. `dht11` — 온습도센서 (`02-라즈베리파이5-바이브코딩-실습교재.md` 4.1과 동일)
...
### A-8. `illuminance` — 조도센서(Cds, `02-라즈베리파이5-바이브코딩-실습교재.md` 4.2와 동일)
...
`02-라즈베리파이5-바이브코딩-실습교재.md` 8장 참고 — 아래는 옵션 A(라즈베리파이 직접 처리) 기준이다.
...
### B-1. `led` — LED (`02-라즈베리파이5-바이브코딩-실습교재.md` 5.2와 동일)
...
### B-6. `servo` — 서보모터(SG-90, `02-라즈베리파이5-바이브코딩-실습교재.md` 5.1과 동일)
...
- 오류가 나면 `02-라즈베리파이5-바이브코딩-실습교재.md` 9.2절(자주 발생하는 오류와 해결 방법)을 먼저 확인한다.
- 핀 번호는 전부 예시다 — 우리 팀 실제 배선과 다르면 프롬프트의 굵은 글씨 부분만 바꿔서 다시 요청한다.
- 여러 부품을 동시에 쓸 때는 GPIO 핀이 서로 겹치지 않는지 반드시 확인한다
  (`02-라즈베리파이5-바이브코딩-실습교재.md` 9.1절, `antigravity-plugin/hooks/`의 핀 중복 점검 훅 참고).
