...
> ⚠️ 팀별 진짜 백엔드/프론트엔드를 `agent-vibe-coding-starter-kit2`로 개발한다면,
> 그 시스템은 경로·인증 헤더·응답 형식이 전혀 다른 자체 계약을 쓴다 — 이 문서를
> 그 시스템에 그대로 적용할 수 없다. 그 경우 라즈베리파이 쪽 REST 연동은 이
> 문서 대신 `agent-vibe-coding-starter-kit2`의 `hardware-agent`/
> `hardware-swap.md` 절차를 따른다 (`antigravity-plugin/README.md`의 "이
> 플러그인의 역할" 참고). 이 문서는 임시 테스트 시스템 자체를 이해하거나,
> 팀이 이 계약을 그대로 진짜 시스템에 채택하기로 별도 합의한 경우에만 쓴다.
