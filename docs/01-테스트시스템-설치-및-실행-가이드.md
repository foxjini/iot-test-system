# 테스트 시스템 설치 및 실행 가이드 (Windows PC)

> 📂 **본편 01 / 팀에서 1명 / 가장 먼저** — Windows PC에 테스트 환경 띄우기 · 전체 목록 [docs/README.md](README.md)
...
**같은 API 계약**(`docs/부록B-REST-API-스펙.md`)을 따르므로, 지금 만든 라즈베리파이 쪽 코드를 거의 그대로
옮겨 쓸 수 있다.
...
> 💡 **'진짜 백엔드로 전환'(본편 03)과 다른 작업입니다.** 여기 7장은 *Mock 파이 → 실제
> 라즈베리파이* 로 바꾸는 것이고, 백엔드는 여전히 이 테스트 시스템을 씁니다. 팀의 진짜
> 백엔드로 갈아타는 것은 [03-진짜-백엔드-연동-확인-매뉴얼.md](03-진짜-백엔드-연동-확인-매뉴얼.md)
> 입니다. 보통 **7장을 먼저, 03을 나중에** 합니다.
