...
**범용 테스트 백엔드 + 프론트엔드 + Mock 라즈베리파이**. 15종 센서/액추에이터를 모두
지원하며, 팀마다 실제 작품에서 쓰는 항목만 골라 구성하고 테스트할 수 있다.

- 센서 8종: 압력센서, 숫자 키패드, 리드 스위치, 불꽃센서, 온습도센서(DHT11),
  초음파센서(HC-SR04), 푸시버튼, 조도센서
...
- **팀마다 센서/액추에이터 구성이 다르다는 점**을 반영해, 15종을 하드코딩하지 않고
