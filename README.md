...
`docs/01-테스트시스템-설치-및-실행-가이드.md` 0번 항목 참고). 항상 **setup 실행 → 방금 생성된 .env 파일 편집 →
run 실행** 순서다.
...
자세한 설치/실행/문제해결은 [`docs/01-테스트시스템-설치-및-실행-가이드.md`](docs/01-테스트시스템-설치-및-실행-가이드.md),
REST API 전체 스펙은 [`docs/부록B-REST-API-스펙.md`](docs/부록B-REST-API-스펙.md) 참고. 팀의 진짜 백엔드가
준비된 뒤 실기기를 실제로 연동하는 절차는
[`docs/03-진짜-백엔드-연동-확인-매뉴얼.md`](docs/03-진짜-백엔드-연동-확인-매뉴얼.md) 참고.
...
- API 경로에는 `device_id`를 유지해, 6개 팀이 테스트 기간 동안 **하나의 테스트
  백엔드 인스턴스를 함께 쓸 수 있도록** 설계했다 (`docs/부록B-REST-API-스펙.md`). 각 팀의 진짜
...
- `mock_pi`는 백엔드에 등록된 컴포넌트 목록을 그대로 읽어와 동작하므로, 대시보드에서
  ...
  (`docs/01-테스트시스템-설치-및-실행-가이드.md` 7번 항목).
