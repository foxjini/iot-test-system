...
전체 계약은 `iot-test-system/docs/부록B-REST-API-스펙.md`가 원본이다. 여기서는 라즈베리파이
쪽 코드를 작성할 때 실제로 필요한 부분만 요약한다. 백엔드가 켜져 있으면
`http://<백엔드 PC IP>:8000/docs`에서 Swagger UI로 직접 확인할 수 있다.

## 인증

라즈베리파이가 호출하는 엔드포인트에는 `X-Device-Key` 헤더에 디바이스 등록 시
발급받은 `api_key`를 담는다 (`.env`에서 읽어온다, 코드에 하드코딩 금지).

예외적으로 `GET /api/devices/{device_id}/components`는 대시보드도 쓰는 조회용이라
인증 없이도 응답한다(`backend/app/auth.py` 참고). 클라이언트에서는 다른 호출과 똑같이
헤더를 붙여도 문제없다.
...
- WebSocket/MQTT는 쓰지 않는다. 폴링 주기는 이 테스트 시스템에서는 1~2초.
  (팀의 진짜 백엔드로 옮길 때는 2~5초를 권장한다 — `docs/03-진짜-백엔드-연동-확인-매뉴얼.md` 2단계 표)
